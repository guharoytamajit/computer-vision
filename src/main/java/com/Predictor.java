...
import com.microsoft.azure.cognitiveservices.vision.customvision.prediction.Predictions.PredictionsClassifyImageDefinitionStages.WithPublishedName;
...
//            ObjectDetection_Sample(trainer, predictor);
    ...
    public static void ImageClassification_Sample(CustomVisionTrainingClient trainClient, CustomVisionPredictionClient predictor) {
        try {
            ...
            WithPublishedName withProjectId = predictor.predictions().classifyImage()
                .withProjectId(id);
    ...
    {
        try {
            return ByteStreams.toByteArray(Predictor.class.getResourceAsStream(folder + "/" + fileName));
        } catch (Exception e) {
            System.out.println(e.getMessage());
            e.printStackTrace();
        }

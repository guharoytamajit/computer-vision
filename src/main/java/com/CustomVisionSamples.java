...
//            ObjectDetection_Sample(trainer, predictor);
    ...
    public static void ImageClassification_Sample(CustomVisionTrainingClient trainClient, CustomVisionPredictionClient predictor) {
        try {
            ...
            Iteration iteration = trainer.trainProject(id, new TrainProjectOptionalParameter());

            while (iteration.status().equals("Training"))
            {
                System.out.println("Training Status: "+ iteration.status());
                Thread.sleep(1000);
                iteration = trainer.getIteration(id, iteration.id());
            }
            System.out.println("Training Status: "+ iteration.status());
            ...
            trainer.publishIteration(id, iteration.id(), publishedModelName, predictionResourceId);
            // </snippet_train>
